...
/* singer options */
%token KW_USE_MAC_MECH
%token KW_HASH_FUNC
%token KW_INITKEY_FILE
...
options_item
  ...
  | KW_USE_MAC_MECH '(' yesno ')' { configuration->use_mac_mechanism = $3; }
	| KW_HASH_FUNC '(' string ')' { configuration->hash_func_name = cfg_ls_hfo_format_value($3); free($3); }
	| KW_INITKEY_FILE '(' string ')' { configuration->initkey_file = g_strdup($3); free($3); }
	;
