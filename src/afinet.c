  ...
  if (self->spoof_source && self->lnet_ctx && msg->saddr && (msg->saddr->sa.sa_family == AF_INET || msg->saddr->sa.sa_family == AF_INET6))
    {
      ...
      GString *msg_line = g_string_sized_new(288); // it was 256, extended allocation - 32 characters
      //GString *fakemac = NULL;

      g_assert((self->super.flags & AFSOCKET_DGRAM) != 0);
      
      log_writer_format_log((LogWriter *) self->super.writer, NULL, msg, msg_line);
