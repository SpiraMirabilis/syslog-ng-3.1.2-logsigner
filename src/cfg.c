...
cfg_new(gchar *fname)
{
  ...
  self->use_mac_mechanism = TRUE;
  self->hash_func_name = LS_HFO_SHA1;
  self->hash_func = compute_sha1;
  self->initkey_file = PATH_SYSLOGNG_INITK;
  // self->seed = NULL;
...
persist_config_free(PersistentConfig *self)
{
  ...
  GlobalInteract *
interactive_input_new(GString *passwd)
{
  GlobalInteract *self = g_new0(GlobalInteract, 1);
  self->passwd = passwd;
  self->seed = NULL;
