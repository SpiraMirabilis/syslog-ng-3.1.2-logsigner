  ...
  if (((s->flags & LTF_DONTCLOSE) == 0) && s->fd != -1)
    {
      ...
      if (s->lemac)
        if (!log_signer_last_key_file_creator(s))
          msg_error("Error writing last key of file",
              evt_tag_str("filename", s->filename->str),
              NULL);
      close(s->fd);
    }
}
...
log_transport_plain_write_method(LogTransport *s, const gpointer buf, gsize buflen)
{
  ...
  LogTransport *
log_transport_plain_new(gint fd, GString *filename, GString *lemac, guint flags)
{
  LogTransportPlain *self = g_new0(LogTransportPlain, 1);

  self->super.fd = fd;
  self->super.filename = filename;
  self->super.cond = 0;
  self->super.flags = flags;
  self->super.lemac = lemac;
