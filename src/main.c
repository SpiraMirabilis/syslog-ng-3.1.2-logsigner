...
static GOptionEntry syslogng_options[] = 
{
  ...
  { "get-pwd",           'p',         0, G_OPTION_ARG_NONE, &get_passwd, "Get decryption password from user", NULL},
#ifdef YYDEBUG
...
main(int argc, char *argv[])
{
  ...
  if (get_passwd)
  {
    interactive_input_new(log_signer_passwd_retriever());
  }
  else
  {
    interactive_input_new(NULL);
  }
  ...
  if (cfg->use_mac_mechanism)
    if (get_passwd)
      log_signer_cfg_refresh(cfg);
    else
    {
      fprintf(stderr, "To activate secure logging use -p option for password passing. Definition of behaviour in configuration file\n");
      return 1;
    }
