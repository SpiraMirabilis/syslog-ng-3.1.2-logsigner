...
affile_dw_init(LogPipe *s)
{
        ...
        !!(self->owner->flags & AFFILE_CREATE_DIRS), FALSE, !!(self->owner->flags & AFFILE_PIPE), &fd))
  {
    ...
    if (!log_pipe_init(self->writer, cfg)) // FIXME: changed NULL to cfg
    {
    ...
    if (cfg->use_mac_mechanism && 
        stat(self->filename->str, &st) == 0 && 
        S_ISREG(st.st_mode))
    {
      init_key = g_string_sized_new(40);
      log_signer_init_key_generator(self->filename, init_key);
      if (st.st_size > 0)
      {
        FILE *log = NULL;
        int lineno = 0;
        if ((log = fopen(self->filename->str, "r")) == NULL)
        {
          msg_error("Error opening file for syntax checking",
              evt_tag_str("filename", self->filename->str),
              evt_tag_errno(EVT_TAG_OSERROR, errno),
              NULL);
          fclose(log);
        ...
        GString *last_entry_key = NULL;
        last_entry_key = g_string_sized_new(64);
        log_signer_get_last_entry_key(last_entry_key, self->filename->str);
        if (!log_signer_malformed_log(cfg, log, self->filename, &lineno, init_key, last_entry_key))
          msg_info("Log file verified successfully",
              evt_tag_str(EVT_TAG_FILENAME, self->filename->str),
              NULL);
        g_string_free(last_entry_key, TRUE);
      }
    }
    write_flags = ((self->owner->flags & AFFILE_FSYNC) ? LTF_FSYNC : 0) | LTF_APPEND;
    log_writer_reopen(self->writer, log_proto_plain_new_client(log_transport_plain_new(fd, self->filename, init_key, write_flags)));
    if (!init_key)
      g_string_free(init_key, TRUE);
  }
  else
  {
