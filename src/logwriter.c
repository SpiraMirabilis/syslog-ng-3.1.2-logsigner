...
log_writer_append_value(GString *result, LogMessage *lm, NVHandle handle, gboolean use_nil, gboolean append_space)
{
  ...
  void
log_writer_format_log(LogWriter *self, GString *lemac, LogMessage *lm, GString *result)
{
  ...
  if ((self->flags & LW_SYSLOG_PROTOCOL) || (self->options->options & LWO_SYSLOG_PROTOCOL))
  {
    ...
    if (cfg->use_mac_mechanism && lemac)
    {
      gchar buf[256];
      gchar macstr[50];

      cfg->hash_func(lemac, (char *) buf);
      g_string_erase(lemac, 0, -1);
      g_string_assign(lemac, buf);
      memset(buf, '\0', sizeof(buf));
      compute_mac(result, lemac, (char *) macstr, cfg->hash_func_name);
      g_string_append_c(result, '\t');
      g_string_append_c(result, '{');
      g_string_append_len(result, macstr, SHA1_MAC_SIZE);
      g_string_append_c(result, '}');
...
log_writer_flush_log(LogWriter *self, LogProto *proto)
{
  ...
  while (num_elements > 0 && !log_writer_throttling(self))
  {
    ...
    log_writer_format_log(self, proto->transport->lemac, lm, line);
